...
const createAnnotationData = datapoint => ({
  note: {
    label: datapoint.run_id + " " + datapoint.seq,
    bgPadding: 5,
    title: trunc(datapoint.id, 100)
  },
  x: datapoint.scaled_loc,
  y: datapoint.data,
  dx: 20,
  dy: 20
});
...
streamingLoaderWorker.onmessage = ({
  ...
  if (finished) {
    ...
    const languageFill = d =>
      webglColor(languageColorScale(hashCode(d.scaled_loc) % 10));
...
streamingLoaderWorker.postMessage("output3.tsv");
...
const chart = fc
  ...
  .webglPlotArea(
    ...
    fc
      .seriesWebglMulti()
      .series([pointSeries])
      .mapping(d => {console.log(d); d.data})
  )
  .svgPlotArea(
